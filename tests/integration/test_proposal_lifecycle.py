...
def ids_from(summary):
    proposal_id = next(line.split()[1] for line in summary.splitlines() if line.startswith("Proposal "))
    prefix = next(line.split(": ")[1] for line in summary.splitlines() if line.startswith("Hash prefix: "))
...
def test_apply_without_authorization_refuses(run_si, workspace):
    ...
    result = run_si("apply-proposal", "--id", proposal_id, "--hash-prefix", prefix, cwd=str(workspace["project"]))
...
def test_full_apply_and_rollback_cycle(run_si, workspace):
    ...
    granted = run_si("capture-expansion", stdin=expansion_event("apply", "%s %s" % (proposal_id, prefix)))
    ...
    rolled = run_si(
        "rollback-mutation", "--id", mutation_id, "--session-id", "session-1", cwd=str(workspace["project"])
    )
...
def test_a_rejected_lesson_is_not_proposed_again(run_si, workspace):
    ...
    run_si("capture-expansion", stdin=expansion_event("reject", proposal_id))
    run_si("reject-proposal", "--id", proposal_id, "--session-id", "session-1", cwd=str(workspace["project"]))
