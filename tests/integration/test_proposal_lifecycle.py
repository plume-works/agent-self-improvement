...
def stage(run_si, workspace, candidate=None):
    args = ["stage-proposal", "--target", str(workspace["target"]),
            "--content-file", str(workspace["content"]),
            "--reason", "This file already holds the project's build commands."]
    if candidate:
...
def ids_from(summary):
    proposal_id = next(line.split()[1] for line in summary.splitlines()
                       if line.startswith("Proposal "))
    prefix = next(line.split(": ")[1] for line in summary.splitlines()
...
def test_full_apply_and_rollback_cycle(run_si, workspace):
    ...
    granted = run_si("capture-expansion",
                     stdin=expansion_event("apply", "%s %s" % (proposal_id, prefix)))
...
def test_rejection_leaves_the_target_unchanged(run_si, workspace):
    ...
    result = run_si("reject-proposal", "--id", proposal_id,
                    "--reason-category", "wrong_scope",
                    "--session-id", "session-1", cwd=str(workspace["project"]))
    assert result.returncode == 0, result.stderr
    assert workspace["target"].read_text() == ORIGINAL
...
def test_a_rejected_lesson_is_not_proposed_again(run_si, workspace):
    ...
    store.write_record(store.CANDIDATES, "cand-1",
                       {"lesson": "Run the suite with make test.",
                        "signal_type": "explicit_correction"}, ttl=3600)
    proposal_id, _ = ids_from(stage(run_si, workspace, candidate="cand-1"))
    run_si("capture-expansion", stdin=expansion_event("reject", proposal_id))
    run_si("reject-proposal", "--id", proposal_id, "--session-id", "session-1",
           cwd=str(workspace["project"]))

    again = run_si("stage-proposal", "--target", str(workspace["target"]),
                   "--content-file", str(workspace["content"]),
                   "--candidate", "cand-1", cwd=str(workspace["project"]))
...
def test_authorization_is_consumed_exactly_once(run_si, workspace):
    ...
    first = run_si("apply-proposal", "--id", proposal_id, "--hash-prefix", prefix,
                   "--session-id", "session-1", cwd=str(workspace["project"]))
    assert first.returncode == 0

    second = run_si("apply-proposal", "--id", proposal_id, "--hash-prefix", prefix,
                    "--session-id", "session-1", cwd=str(workspace["project"]))
...
def test_staging_outside_the_allowlist_refuses(run_si, workspace, tmp_path):
    ...
    result = run_si("stage-proposal", "--target", str(outside),
                    "--content-file", str(workspace["content"]),
                    cwd=str(workspace["project"]))
    assert result.returncode == 1
...
def test_find_owners_reports_only_allowlisted_paths(run_si, workspace):
    ...
    assert any(entry["path"] == str(workspace["target"]) for entry in
               report["owners"])
...
def test_skills_are_discoverable_and_restrict_their_tools():
    ...
    for name in ("improve", "apply", "reject", "rollback"):
        ...
        allowed = next(line for line in text.splitlines()
