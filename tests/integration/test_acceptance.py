...
def test_1_reflection_only_follows_a_supported_signal(run_si, workspace, fake_reviewer):
    ...
    assert not os.path.exists(fake_reviewer.argv_file), "the reviewer must not run without a signal"
...
def test_5_authorization_binds_one_exact_immutable_proposal(run_si, workspace):
    ...
    second, _ = stage(run_si, workspace, content="# Project\n\n- Build with make.\n- Something else.\n")
...
def test_7a_path_attacks_fail_without_writing(run_si, workspace, attack):
    ...
    result = run_si("stage-proposal", "--target", target, "--content-file", str(content), cwd=str(workspace["project"]))
...
def test_10_persisted_state_contains_no_sensitive_body(run_si, workspace, fake_reviewer, state_root):
    """The strongest privacy claim, checked across every file that survives."""
    fake_reviewer.mode("propose")
    secret_prompt = "no, use make test instead of pytest; the token is ghp_abcdefghijklmnopqrstuvwxyz0123"
