...
def corrected(workspace):
    capture.record_prompt({**workspace["stop"],
                           "prompt": "no, use make test instead of pytest"})
...
def candidate_id_from(stderr):
    return next(word.strip(".") for word in stderr.split()
...
def stage(run_si, workspace, candidate=None, content=None):
    ...
    args = ["stage-proposal", "--target", str(workspace["target"]),
            "--content-file", str(path), "--reason", "It already holds build commands."]
    if candidate:
        args += ["--candidate", candidate]
    result = run_si(*args, cwd=str(workspace["project"]))
    assert result.returncode == 0, result.stderr
    lines = result.stdout.splitlines()
    return (next(line.split()[1] for line in lines if line.startswith("Proposal ")),
            next(line.split(": ")[1] for line in lines
                 if line.startswith("Hash prefix: ")))


def authorize(run_si, operation, args, session="session-1"):
    return run_si("capture-expansion", stdin=json.dumps({
...
def test_1_reflection_only_follows_a_supported_signal(run_si, workspace,
                                                      fake_reviewer):
...
def test_2_a_no_lesson_turn_is_silent_and_stages_nothing(run_si, workspace,
                                                         fake_reviewer):
...
def test_3_the_reviewer_is_independent_and_cannot_mutate(run_si, workspace,
                                                         fake_reviewer):
    fake_reviewer.mode("propose")
...
def test_4_routing_searches_existing_owners_before_creating(run_si, workspace):
    result = run_si("find-owners", "--query", "test command",
...
def test_5_authorization_binds_one_exact_immutable_proposal(run_si, workspace):
    ...
    second, _ = stage(run_si, workspace,
                      content="# Project\n\n- Build with make.\n- Something else.\n")

    authorize(run_si, "apply", "%s %s" % (first, first_prefix))

    # The authorization names one proposal; it cannot install the other.
    wrong = run_si("apply-proposal", "--id", second, "--hash-prefix", first_prefix,
                   "--session-id", "session-1", cwd=str(workspace["project"]))
    assert wrong.returncode == 1
    assert "no_matching_authorization" in wrong.stderr
    assert workspace["target"].read_text() == "# Project\n\n- Build with make.\n"


def test_6_approval_applies_only_reviewed_bytes_to_one_artifact(run_si, workspace,
                                                               tmp_path):
    before = {}
    ...
    result = run_si("apply-proposal", "--id", proposal_id, "--hash-prefix", prefix,
                    "--session-id", "session-1", cwd=str(workspace["project"]))
    assert result.returncode == 0, result.stderr
...
def test_7a_path_attacks_fail_without_writing(run_si, workspace, attack):
    ...
    result = run_si("stage-proposal", "--target", target,
                    "--content-file", str(content), cwd=str(workspace["project"]))
...
def test_7c_an_interrupted_mutation_blocks_until_reconciled(run_si, workspace):
    proposal_id, prefix = stage(run_si, workspace)
    paths.atomic_write(paths.state_path("inflight.json"), json.dumps({
...
def test_8_a_fresh_process_discovers_the_applied_artifact(run_si, workspace):
    proposal_id, prefix = stage(run_si, workspace)
    authorize(run_si, "apply", "%s %s" % (proposal_id, prefix))
    run_si("apply-proposal", "--id", proposal_id, "--hash-prefix", prefix,
           "--session-id", "session-1", cwd=str(workspace["project"]))
    ...
    probe = subprocess.run(
        [sys.executable, "-c",
         "import os,sys;p=os.path.join(sys.argv[1],'CLAUDE.md');"
...
def test_9_verified_rollback_restores_the_preimage(run_si, workspace):
    ...
    proposal_id, prefix = stage(run_si, workspace)
    authorize(run_si, "apply", "%s %s" % (proposal_id, prefix))
    applied = run_si("apply-proposal", "--id", proposal_id, "--hash-prefix", prefix,
                     "--session-id", "session-1", cwd=str(workspace["project"]))
    mutation_id = applied.stdout.split("Mutation ")[1].split(".")[0]

    authorize(run_si, "rollback", mutation_id)
    rolled = run_si("rollback-mutation", "--id", mutation_id,
                    "--session-id", "session-1", cwd=str(workspace["project"]))
    assert rolled.returncode == 0, rolled.stderr
    assert workspace["target"].read_text() == original


def test_10_persisted_state_contains_no_sensitive_body(run_si, workspace,
                                                       fake_reviewer, state_root):
    """The strongest privacy claim, checked across every file that survives."""
    fake_reviewer.mode("propose")
    secret_prompt = ("no, use make test instead of pytest; "
                     "the token is ghp_abcdefghijklmnopqrstuvwxyz0123")
    capture.record_prompt({**workspace["stop"], "prompt": secret_prompt})
    capture.record_tool_failure({
        **workspace["stop"], "tool_name": "Bash",
        "tool_input": {"command": "curl -H 'Authorization: Bearer sk-livesecret123456'"
...
def test_the_packaged_plugin_declares_every_hook_it_implements():
    ...
    assert stop["asyncRewake"] is True, \
        "the Stop hook must not delay the user's response"
