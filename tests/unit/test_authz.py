...
@pytest.mark.parametrize("name", [
    "apply", "self-improve:apply", "/apply", "APPLY", " apply ",
])
...
@pytest.mark.parametrize("name", [
    "improve", "deploy", "other-plugin:apply", "", None, "applying",
])
...
@pytest.mark.parametrize(("operation", "args", "expected"), [
    (authz.APPLY, "prop-1 abc123def456",
...
def test_a_granted_authorization_is_consumable_once(state_root):
    authz.grant(EVENT, authz.APPLY,
                {"proposal_id": "prop-abc123", "hash_prefix": "0123456789ab"})

    claimed = authz.consume(authz.APPLY, session_id="session-1",
                            proposal_id="prop-abc123",
                            hash_prefix="0123456789ab")
    assert claimed["operation"] == authz.APPLY

    with pytest.raises(authz.AuthorizationError):
        authz.consume(authz.APPLY, session_id="session-1",
                      proposal_id="prop-abc123", hash_prefix="0123456789ab")
...
def test_consumption_requires_the_matching_proposal(state_root):
    authz.grant(EVENT, authz.APPLY,
                {"proposal_id": "prop-abc123", "hash_prefix": "0123456789ab"})
    with pytest.raises(authz.AuthorizationError):
        authz.consume(authz.APPLY, proposal_id="prop-different",
                      hash_prefix="0123456789ab")
...
def test_consumption_requires_the_matching_hash_prefix(state_root):
    ...
    authz.grant(EVENT, authz.APPLY,
                {"proposal_id": "prop-abc123", "hash_prefix": "0123456789ab"})
    with pytest.raises(authz.AuthorizationError):
        authz.consume(authz.APPLY, proposal_id="prop-abc123",
...
def test_consumption_is_scoped_to_the_granting_session(state_root):
    authz.grant(EVENT, authz.APPLY,
                {"proposal_id": "prop-abc123", "hash_prefix": "0123456789ab"})
    with pytest.raises(authz.AuthorizationError):
        authz.consume(authz.APPLY, session_id="a-different-session",
                      proposal_id="prop-abc123", hash_prefix="0123456789ab")
...
def test_the_record_is_removed_from_disk_when_claimed(state_root):
    record = authz.grant(EVENT, authz.APPLY,
                         {"proposal_id": "prop-abc123",
                          "hash_prefix": "0123456789ab"})
    authz.consume(authz.APPLY, proposal_id="prop-abc123",
                  hash_prefix="0123456789ab")
    assert store.read_record(store.AUTHORIZATIONS, record["nonce"],
