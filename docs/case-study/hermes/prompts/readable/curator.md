# Human-readable curator prompt copy

This is a rendered reading copy of the Python snapshot at
[`../python/curator.py`](../python/curator.py). Python strings are joined. Runtime
composition is shown with explicit placeholders:

- `<OPTIONAL PRUNE-BUILTINS OVERRIDE>` is empty unless that mode is enabled;
- `<DYNAMIC CURATOR CANDIDATE LIST>` is generated from eligible skill metadata;
- the dry-run banner is prepended only in preview mode.
