...
- **Status:** Implemented; **verified except section 6.1**. The harness is `tests/smoke/pty_harness.py` and `tests/smoke/test_wake_pty.py`, run with `make wake`. Its model-free self-checks and its harness self-checks (`make test-harness`) pass. Both live checks were observed passing together (`make wake`, 2026-08-01, Claude Code 2.1.220): the wake arrived at an idle session for `cand-12f3c9117d4c`, and the negative control stored `cand-d5552db5fde5` and saw no wake when the `Stop` hook cannot signal. Criteria 6.2 to 6.5 have been observed. Criterion 6.1 asks for an arriving wake detected in ten consecutive runs: `make wake-repeat` completed ten consecutive runs with no failure (2026-08-02), but one of the ten wake checks skipped on a review that stored no candidate, so nine of the ten reached the assertion that a wake arrived. That criterion stays partially verified until ten do. Five of the twenty checks across those runs skipped for that reason; the other four were the negative control, carried as [Spec-0005](0005-reviewer-decline-asymmetry.md).
...
That is not a rare case. Replaying a reconstruction of the scripted exchange's evidence bundle through the real reviewer 56 times declined 3 of them with `transient_state`, on a bundle the other 53 proposed from. Live it is commoner still: the ten-run loop of section 8 declined 5 of 20. Failing on a decline would have made a red run the _expected_ outcome of `make wake`, every one of them pointing at a wake that was never in question. A harness that cries wolf at that rate stops being read.
...
4. ~~a run in which the budget is _not_ consumed~~ — evidence that three minutes is genuinely generous rather than the thing being measured. **Observed 2026-08-01**: the positive check completed and closed its session 32s into a 180s budget, having watched for the wake and found it rather than having waited out a timeout.
