...
That is not a rare case. Replaying a reconstruction of the scripted exchange's evidence bundle through the real reviewer 56 times declined 3 of them with `transient_state`, on a bundle the other 53 proposed from. Live, it is far commoner still: the ten-run loop of section 8 declined 5 of 20, and [Spec-0005](0005-reviewer-decline-asymmetry.md) sets out why the offline figure understates it. Failing on a decline would have made a red run the *expected* outcome of `make wake`, every one of them pointing at a wake that was never in question. A harness that cries wolf at that rate stops being read.
...
3. ~~`make wake-repeat` completing ten consecutive runs without a failure~~ — **observed 2026-08-02.** Ten consecutive runs, no failure, each between 2m05s and 3m08s, and each in its own directory under `test-runs/` so all ten remained readable afterwards. Of the twenty checks, fifteen ran to their assertions and five skipped on a reviewer that stored no candidate — see [8.3](#83-the-reviewer-declines-far-more-often-on-the-control) and, for the question itself, [Spec-0005](0005-reviewer-decline-asymmetry.md); and
...
The ten-run loop of item 3 is the first data set large enough to say which check the reviewer declines on: the wake check proposed 9 times in 10, the negative control 6 in 10. With the three earlier live runs, the control has declined seven times and the wake check once, on an exchange scripted identically for both.

No mechanism has been identified. The finding is the harness's, but the question is the reviewer's, and it is carried in full — the measurement, the hypotheses since eliminated, and what would settle it — in **[Spec-0005](0005-reviewer-decline-asymmetry.md)**, so that this specification can close on the criteria it set for itself rather than staying open on a question it only discovered.

What it means for reading a run of this harness is the whole of what belongs here: **a decline on the control is the common case, not the surprising one**, and it says nothing about the wake. That is why such a run skips rather than fails.
