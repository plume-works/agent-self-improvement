...
The offline suite passes, and nine of the ten smoke checks pass. The tenth — an asynchronous review waking an idle session — cannot be observed in print mode, where a turn ends at `result` and there is no idle session to wake. The pseudo-terminal harness of [Spec-0002](docs/specs/0002-pty-wake-harness.md) automates it behind `make wake`. Both of its live checks have been observed passing in one run (2026-08-01): a wake arrived at an idle session, and a suppressed wake was detected as absent. What has not been observed is the stability that spec asks for — `make wake-repeat` has never completed its ten consecutive runs — so the interactive question in `make smoke` is still worth answering.
